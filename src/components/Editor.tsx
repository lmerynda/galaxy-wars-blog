...
function upload(role: Role, file: File, onProgress: (value: number) => void) {
...
export function Editor({ initial }: { initial: Post }) {
  ...
  async function selectImage(role: Role, file?: File) {
    ...
    setProgress(0);
    try {
      const image = await upload(role, file, setProgress);
  ...
  async function submit() {
    ...
    try {
      ...
      if (result.error) setError(result.error);
        ...
        setMessage("Saved. Your update is live.");
        window.history.replaceState(
          null,
          "",
          `/admin/posts/${result.post.id}/edit`,
        );
      }
    } catch {
      setError(
  ...
  return (
    <>
      ...
      <div className="admin-heading editor-heading">
        <div>
          <p className="eyebrow">
            Owner studio / {saved.version ? "Edit entry" : "New entry"}
          </p>
          <h1>
      ...
      {saved.version > 0 && <Discussion postId={saved.id} admin />}
      <div className="save-bar">
        <div aria-live="polite">
          ...
          <p className="muted">
            {message ||
              (saved.version
                ? `Grouped under ${formatDay(saved.publishedDay!)}.`
                : "Save to share this entry on the public log.")}
          </p>
          {saved.version > 0 && (
        ...
        <div className="actions">
          <button
            ...
            onClick={() => void submit()}
          >
            {saved.version ? "Save changes →" : "Save entry →"}
          </button>
        </div>
      </div>
    </>
